...
void t(fstream& file1, fstream& file2, vector<string>& expences, vector<streampos>& pos1, vector<streampos>& pos2)
{
	...
	while (true)
	{
		...
		cout << "customer3 appealed to " << expences[rkey] << " in cash " << nfile << '\n';
		ReleaseMutex(m);
		if (nfile == 1)
		{
...
int main(int argc, char* argv[])
{
	...
	ReadCash3 = OpenEvent(EVENT_MODIFY_STATE, FALSE, L"CASH3");
	if (ReadCash3)
		ResetEvent(ReadCash3);
	ReadCash1 = OpenEvent(SYNCHRONIZE, FALSE, L"CASH1");
	ReadCash2 = OpenEvent(SYNCHRONIZE, FALSE, L"CASH2");
	...
	WaitForSingleObject(ReadCash2, INFINITE);
